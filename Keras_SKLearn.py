...
if data_set=='STS' or data_set=='MR':
    ...
    features_lst = [negScore, posScore, subjScore, pposScore, nnegScore, valenceScore,arousalScore,dominanceScore,
    ...
    if ade_exact==1:
        ...
        features_lst = [negScore, posScore, subjScore, pposScore, nnegScore, adeNoStopWords,
                        moreGoodScore, lessBadScore, lessGoodScore, moreBadScore, wordLength, wordOrder]
...
model.fit(X[train], Y[train], epochs=epochs_num, batch_size=batch_size, verbose=1)#, callbacks=[metrics], callbacks=[metrics,csv_logger]
model.evaluate(X[test],  Y[test], verbose=1)
