...
class Metrics(Callback):
    def __init__(self, split_id, x_val, y_val, batch_size,file_name,features_num,filters_num):
        ...
        features_num_formatted = ("{0}-features".format(features_num))
        ...
        csv_writer.writerow(
            ["Epoch", "Accuracy", 'Loss', 'Avg Precision Weighted', 'Avg Precision Macro', 'Avg Precision Micro',
             ...
             "Negative Recall",
             "Positive f-score", "Negative f-score", "loss", "Elapsed Time"])
...
X, Y = data_helpers.load_data_and_y_labels("./data/rt-polaritydata/rt-polarity.pos",
...
labels = to_categorical(numpy.asarray(Y))
...
# embedding_matrix = numpy.zeros((num_words + 1, EMBEDDING_DIM))
...
for train, test in kfold.split(data, Y):
    ...
    # create model
    model = model_name(features_num,hidden_dims=128, kernel_size=2,pool_size=2, use_embedding=True)#create_model(features_num)
    file_name="%s-features-%s-filters.csv" % (features_num, num_filters)

    metrics=Metrics( index, data[test], Y[test], batch_size,file_name,features_num, num_filters)
    history=model.fit(data[train], Y[train], epochs=epochs_num, batch_size=batch_size, verbose=1, callbacks=[metrics])#, callbacks=[metrics], callbacks=[metrics,csv_logger]
    ...
    # evaluate the model
    scores = model.evaluate(data[test],  Y[test], verbose=1)

    ypreds = model.predict_classes(data[test], batch_size=batch_size, verbose=1)
